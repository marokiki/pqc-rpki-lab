...
文献値とサイズモデルでは**Falcon-512が最も有力な対抗候補**である。ただし、
この証拠スナップショットではFalcon primitiveを実行できていない。FN-DSA、
X.509、CMS、provider、HSM、validator実装が成熟し、同一条件のローカル測定が
得られた時点で主候補を再判定する。

| 用途 | 提案 |
|---|---|
| 最初のSIDROPS実験用MTI | ML-DSA-65 |
| 文献・サイズモデル上の高優先度比較 | Falcon-512 |
...
| ML-DSA-44 | 2 | 1,312 B | 2,420 B | measured comparison; profile-excluded |
...
RSA、ML-DSA、SLH-DSA、ML-DSA-44の比較行は、各操作をOpenSSL CLIの1 subprocess
として実行する。値にはprocess起動、provider初期化、引数処理、file I/O、暗号操作を
含むため、純粋な暗号cycleではない。同じ`comparable_group`内だけを比較でき、verify値の
横並びは主に共通CLI経路を表す。optional方式のoqs-python測定はin-processの別比較群である。
...
最新値は`results/primitive-bench.csv`を一次資料とする。Falcon-512のローカルprimitive
測定は`unsupported`である。Falcon署名は可変長なのでrepository推定には最大長を使う。

SLH-DSA-128s/192sのsignは同じCLI比較群でも数百msであり、CAの大量再発行に不利である。
...
- **文献・モデル上の最優先比較候補:** Falcon-512。ローカル実測とprofile相互運用が必要。
- **比較用:** ML-DSA-44、ML-DSA-87、Falcon-1024。
...
- Dirk Doesburg, *Post-Quantum Cryptography for the RPKI*, Master's thesis,
  Radboud University, 27 June 2025
